...
Formula: `value ~ drought * host * site + (1|block))`
...
sterile.full <- readRDS("./Output/phyloseq_objects/16S_clean_phyloseq_object_w_guilds.RDS") %>% 
...
# bact_inoc@tax_table[,"Phylum"][is.na(bact_inoc@tax_table[,"Phylum"])] <- "Undetermined"
...
m <- readRDS("./Output/16S_alpha_diversity_lmermod_model.RDS") 
formula(m)
```
...
```{r}
df <- readRDS("./Output/16S_alpha_diversity_lmermod_table.RDS") 
df %>% 
  mutate(across(where(is.numeric),function(x){round(x,3)})) %>% 
  kable() %>% kable_classic(lightable_options = 'hover') %>% 
  row_spec(which(df$p.value <= 0.05), bold=TRUE) %>% 
...
formula(m)
df <- readRDS("./Output/ITS_alpha_diversity_lmermod_table.RDS") 
df %>% 
  mutate(across(where(is.numeric),function(x){round(x,3)})) %>% 
  kable() %>% kable_classic(lightable_options = 'hover') %>% 
  row_spec(which(df$p.value <= 0.05), bold=TRUE) %>% 
...
**Figure 4**
```{r}
site1.inoc.full <- readRDS("./Output/phyloseq_objects/18S_site1.inoc.RDS")
site2.inoc.full <- readRDS("./Output/phyloseq_objects/18S_site2.inoc.RDS")
site3.inoc.full <- readRDS("./Output/phyloseq_objects/18S_site3.inoc.RDS")
site4.inoc.full <- readRDS("./Output/phyloseq_objects/18S_site4.inoc.RDS")
site5.inoc.full <- readRDS("./Output/phyloseq_objects/18S_site5.inoc.RDS")
site6.inoc.full <- readRDS("./Output/phyloseq_objects/18S_site6.inoc.RDS")
...
bact_inoc <- bact_inoc %>% 
  subset_samples(!is.na("other_frompreviouscolumn"))
bact_inoc %>% 
  subset_taxa(taxa_sums(bact_inoc) > 0 & Subdivision == "Glomeromycotina") %>% 
  merge_samples("other_frompreviouscolumn") %>% 
  transform_sample_counts(function(x){x/sum(x)}) %>% 
...
**Table 4**
```{r}
readRDS("./Output/figs/18S_Shannon_vs_plant_site.RDS")
readRDS("./Output/figs/18S_Shannon_vs_plant_burnfreq.RDS")
readRDS("./Output/figs/18S_Shannon_vs_plant_drought.RDS")

formula(m)
df <- readRDS("./Output/18S_alpha_diversity_lmermod_table.RDS") 
df %>% 
  mutate(across(where(is.numeric),function(x){round(x,3)})) %>% 
  kable() %>% kable_classic(lightable_options = 'hover') %>% 
  row_spec(which(df$p.value <= 0.05), bold=TRUE) %>% 
...
**Figure 6**
```{r}
readRDS("./Output/figs/16S_Barplot_Phylum.RDS") + labs(title = "Root 16S Overview")
```
...
**Figure 9**
```{r}
readRDS("./Output/figs/ITS_Barplot_Class.RDS") + labs(title = "Root ITS Overview")
```
...
**Figure 11**
```{r}
p1 <- readRDS("./Output/figs/18S_inoc_plot_1.RDS") + labs(title = "Inoculum 1")
p2 <- readRDS("./Output/figs/18S_inoc_plot_2.RDS") + labs(title = "Inoculum 2")
p3 <- readRDS("./Output/figs/18S_inoc_plot_3.RDS") + labs(title = "Inoculum 3")
p4 <- readRDS("./Output/figs/18S_inoc_plot_4.RDS") + labs(title = "Inoculum 4")
p5 <- readRDS("./Output/figs/18S_inoc_plot_5.RDS") + labs(title = "Inoculum 5")
p6 <- readRDS("./Output/figs/18S_inoc_plot_6.RDS") + labs(title = "Inoculum 6")
...
**Figure 12**
```{r}
readRDS("./Output/figs/18S_Barplot_Genus.RDS") + labs(title = "Root AMF Overview")
```
<br>
**Not all taxa in the inoculae were subsequently found in the root samples**
...
**Figure 13**
```{r}
include_graphics("./Output/figs/18S_successful_vs_unsuccessful_taxa_breakdown.png")
```
...
**Table 5**
```{r}
df <- readRDS("./Output/16S_Guild_Model_Output.RDS") 
m <- readRDS("./Output/16S_Pathogen_Model_Snowbrush.RDS")
...
**Table 6**
```{r}
df <- readRDS("./Output/ITS_Guild_Model_Output.RDS") 
df %>% 
  # arrange(species) %>% 
  kable %>% 
  row_spec(which(df$p.value <= 0.05), bold=TRUE) %>% 
...
**Figure 19**
```{r, fig.width=8,fig.height=6}
# readRDS("./Output/figs/18S_Ordination_Plots_Host.RDS") + theme(legend.position = 'bottom')
readRDS("./Output/figs/18S_Ordination_Plots_Drought.RDS") + theme(legend.position = 'bottom')
readRDS("./Output/figs/18S_Ordination_Plots_Fire.RDS") + theme(legend.position = 'bottom')
readRDS("./Output/figs/18S_Ordination_Plots_Site.RDS") + theme(legend.position = 'bottom')
```
...
readRDS("./Output/18S_Permanova_Table.RDS") %>% 
  kable() %>% kable_classic(lightable_options = 'hover')
```
...
**Figure 24**
```{r,fig.width=12,fig.height=6}
include_graphics("./Output/figs/18S_BBDML_Plot.png")
...
**Figure 25**
```{r, fig.width=12,fig.height=8}
readRDS("./Output/figs/18S_Important_Taxa_Abundances_site.RDS")
readRDS("./Output/figs/18S_Important_Taxa_Abundances_fire.RDS")
readRDS("./Output/figs/18S_Important_Taxa_Abundances_drought.RDS")
