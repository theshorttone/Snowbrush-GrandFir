# -----------------------------------------------------------------------------#
# Determining ASV transplantation success (16S)
...
saveRDS(inoc_plot_2,"./Output/figs/16S_inoc_plot_2.RDS")
...
saveRDS(inoc_plot_3,"./Output/figs/16S_inoc_plot_3.RDS")
...
saveRDS(inoc_plot_4,"./Output/figs/16S_inoc_plot_4.RDS")
...
saveRDS(inoc_plot_5,"./Output/figs/16S_inoc_plot_5.RDS")
...
melted <- 
  ...
  mutate(success=ifelse(success == "FALSE","Unsuccessful","Successful"),
                        Phylum = case_when(is.na(Phylum) ~ "Undetermined",
                                           TRUE ~ Phylum)) %>% 
...
melted <- melted %>% mutate(Phylum = case_when(Phylum %in% phy_order[1:10] ~ Phylum,
                                               TRUE ~ "Other"))
melted <- melted %>% mutate(Phylum = factor(Phylum, levels = c(phy_order[1:10],"Other")))
...
p
ggsave("./Output/figs/16S_successful_vs_unsuccessful_taxa_breakdown_withlines.png",width = 12,height = 8)
ggsave("./Output/figs/manuscript_versions/16S_successful_vs_unsuccessful_taxa_breakdown_withlines.tiff",width = 12,height = 8,dpi=500)
saveRDS(p,"./Output/figs/16S_successful_vs_unsuccessful_taxa_breakdown_withlines.RDS")
...
successful_16S <- 
  ...
  as.data.frame() 
successful_16S[is.na(successful_16S)] <- "Undetermined"
write_csv(successful_16S,"./Output/16S_Successful_dataframe.csv")
...
unsuccessful_16S <- 
  ...
  lapply(function(x){str_remove(x,".__") %>% str_remove(".  ")}) %>% 
  as.data.frame() 
unsuccessful_16S[is.na(unsuccessful_16S)] <- "Undetermined"
write_csv(unsuccessful_16S,"./Output/16S_Unsuccessful_dataframe.csv")
...
bact_merged %>% 
  ...
  write_csv("./Output/16S_Successfully_Transplanted_Taxa.csv")
...
successful_taxa <- bact_successful %>% subset_taxa(taxa_sums(bact_successful) > 0) %>% taxa_names

# Get full list of ASVs from Sterile Control root samples
x <- readRDS("Output/phyloseq_objects/16S_clean_phyloseq_object.RDS")
...
data.frame(successful_asv = successful_taxa,
           taxonomy = corncob::otu_to_taxonomy(successful_taxa,bact),
           found_in_sterile = successful_taxa %in% sterile_taxa) %>% 
  write_csv("./Output/16S_successful_taxa_comparison_w_sterile.csv")
...
saveRDS(successful_taxa,"./Output/Successful_ASVs_16S.RDS")
