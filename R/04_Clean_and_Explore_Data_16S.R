...
# DATA ####
ps <- readRDS("./Output/16S_ps_not-cleaned.RDS") # change to non-phylogeny stuff
...
site1.inoc <- ps %>% subset_samples(other_frompreviouscolumn == "Site1")
site2.inoc <- ps %>% subset_samples(other_frompreviouscolumn == "Site2")
site3.inoc <- ps %>% subset_samples(other_frompreviouscolumn == "Site3")
site4.inoc <- ps %>% subset_samples(other_frompreviouscolumn == "Site4")
site5.inoc <- ps %>% subset_samples(other_frompreviouscolumn == "Site5")
site6.inoc <- ps %>% subset_samples(other_frompreviouscolumn == "Site6")
# use these against the full ps community for each inoc source
