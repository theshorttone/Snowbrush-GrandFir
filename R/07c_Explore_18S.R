...
extract_bbdml_model <- function(da_analysis,sig.taxa=NA){
  ...
  x$taxonomy <- corncob::otu_to_taxonomy(x$asv,ps)
...
## data ####
ps <- readRDS("./Output/phyloseq_objects/18S_clean_phyloseq_object.RDS") 
inoc <- readRDS("./Output/phyloseq_objects/18S_inoculum_samples_clean_phyloseq_object.RDS")
ps@sam_data$host <- ifelse(ps@sam_data$species == "Snowbrush", "Ceanothus velutinus","Abies grandis")
ps@sam_data$inoculum_site <- ps@sam_data$inoculum_site %>% factor(levels = c("Sterile",'1','2','3','4','5','6'))
...
microbiome::meta(ps) %>% 
  ...
  lmer(data = .,
       formula = value ~ drought * inoculum_site + (1|block)) %>% 
  broom.mixed::tidy() %>% 
  saveRDS("./Output/Plant_Health_Measures_Overview_Model_table_SnowbrushOnly.RDS")
...
melted %>% 
  mutate(Sample = factor(Sample,levels=sample_order),
         Genus = ifelse(is.na(Genus),"Undetermined",str_remove(Genus,"g__"))) %>% 
  ggplot(aes(x=Sample,y=Abundance,fill=Genus)) +
  geom_col() +
  facet_wrap(~inoculum_site,nrow = 1,scales = 'free_x') +
  theme_bw() +
  theme(axis.text.x = element_text(angle=90,hjust=1,vjust=.5,size=6,face='bold'),
...
p
ggsave("./Output/figs/18S_Barplot_Genus.png",height = 8,width = 10,dpi=400)
...
alpha_long_transformed %>%
  ...
  theme(strip.text.x = element_text(face='bold.italic',size=18),
  ...
  scale_color_manual(values = drought_colors)
p
saveRDS(p,"./Output/figs/18S_Shannon_vs_plant_drought.RDS")
ggsave("./Output/figs/18S_Shannon_vs_plant_drought.png",height = 4,width = 10)
...
saveRDS(p,"./Output/figs/18S_Shannon_vs_plant_burnfreq.RDS")
p
ggsave("./Output/figs/18S_Shannon_vs_plant_burnfreq.png",height = 4,width = 10)

### inoc site ####
p <- alpha_long_transformed %>% 
...
saveRDS(p,"./Output/figs/18S_Shannon_vs_plant_site.RDS")
p
ggsave("./Output/figs/18S_Shannon_vs_plant_site.png",height = 4,width = 10)
...
alpha_mod <- alpha_long_transformed %>% 
  lmer(formula = overall_transformed_plant_value ~ Shannon * drought * inoculum_site + (1|block),
       data = .)
summary(alpha_mod)
saveRDS(broom.mixed::tidy(alpha_mod),"./Output/18S_alpha_diversity_lmermod_table.RDS")

alpha_df %>% 
  mutate(height = scale(height)) %>% 
  lmer(formula = height ~ Shannon * drought * inoculum_site + (1|block), data = .) %>% 
  summary()
...
#   theme_minimal()
# p
# saveRDS(p,"./Output/figs/18S_burn_frequency_and_alpha-div.RDS")
# ggsave("./Output/18S_burn_frequency_and_alpha-div.png",height = 4,width = 6,dpi=300)
# 
# p <- 
# alpha_df %>% 
#   pivot_longer(c(Observed,Shannon,Simpson),names_to = "Measure") %>% 
...
#   theme_minimal()
# p
# saveRDS(p,"./Output/figs/18S_inoc_source_and_alpha-div.RDS")
# ggsave("./Output/18S_inoc_source_and_alpha-div.png",height = 4,width = 6,dpi=300)
...
ord_rda <- ps %>% 
  transform_sample_counts(function(x){x/sum(x)}) %>% 
  ordinate(method = "RDA",distance = "bray")
...
p1; saveRDS(p1,"./Output/figs/18S_Ordination_Plots_Drought.RDS")
...
p2; saveRDS(p2,"./Output/figs/18S_Ordination_Plots_Site.RDS")
...
p4; saveRDS(p4,"./Output/figs/18S_Ordination_Plots_Fire.RDS")
...
permanova_results; saveRDS(permanova_results,"./Output/18S_Permanova_Table.RDS")
...
ggsave("./Output/figs/18S_VIP_Plot_drought.png",width = 8,height = 4)
...
ggsave("./Output/figs/18S_VIP_Plot_fire.png",width = 8,height = 4)
...
saveRDS(p,"./Output/figs/18S_Important_Taxa_Abundances_fire.RDS")
# ggsave("./Output/figs/18S_Important_Taxa_Abundances_fire.png",width = 8,height = 5)
...
ps_genus_drought_topten %>% 
  ...
  labs(fill="Drought")
saveRDS(p,"./Output/figs/18S_Important_Taxa_Abundances_drought.RDS")
# ggsave("./Output/figs/18S_Important_Taxa_Abundances_drought.png",width = 8,height = 5)
...
saveRDS(p3,"./Output/figs/18S_heatmap_mostabund.RDS")
