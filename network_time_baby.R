...
bind_phyloseq_taxa_simple <- function(ps1, ps2) {
  stopifnot(inherits(ps1, "phyloseq"), inherits(ps2, "phyloseq"))
  
  # hard check: same samples, same order
  if (!identical(sample_names(ps1), sample_names(ps2))) {
    stop("sample_names(ps1) and sample_names(ps2) are not identical (including order). Reorder/prune upstream.")
  }
  
  # extract OTU matrices
  m1 <- as(otu_table(ps1), "matrix")
  m2 <- as(otu_table(ps2), "matrix")
  
  # bind taxa depending on orientation
  if (taxa_are_rows(ps1) != taxa_are_rows(ps2)) {
    stop("taxa_are_rows differs between ps1 and ps2. Make them match upstream.")
  }
  
  if (taxa_are_rows(ps1)) {
    otu_full <- rbind(m1, m2)
    taxa_rows <- TRUE
  } else {
    otu_full <- cbind(m1, m2)
    taxa_rows <- FALSE
  }
  
  # bind taxonomy rows (assumes same tax_table columns)
  tt1 <- as(tax_table(ps1), "matrix")
  tt2 <- as(tax_table(ps2), "matrix")
  if (!identical(colnames(tt1), colnames(tt2))) {
    stop("tax_table column names differ between ps1 and ps2. Align them upstream.")
  }
  tt_full <- rbind(tt1, tt2)
  
  phyloseq(
    otu_table(otu_full, taxa_are_rows = taxa_rows),
    sample_data(ps1),          # metadata assumed identical
    tax_table(tt_full)
  )
}
...
bact <- prune_taxa(taxa_sums(bact) > 0, bact)

fung <- readRDS("./Output/phyloseq_objects/ITS_clean_phyloseq_object_w_guilds.RDS")
fung <- prune_taxa(taxa_sums(fung) > 0, fung)
a <- as.data.frame(sam_data(fung))


amf <- readRDS("./Output/phyloseq_objects/18S_clean_phyloseq_object.RDS")
tt <- (tax_table(amf))

tt <- tt %>% subset_taxa("Phylum" == "p__Glomeromycota") 

amf <- readRDS("./Output/phyloseq_objects/18S_clean_phyloseq_object.RDS")

#amf <- amf %>% subset_taxa(grepl("Glomeromycota", Phylum))
 #this doen't work for me 
amf <- amf %>% subset_taxa(taxa_sums(amf) > 0)

# join them for 'full' microbiome (16S + ITS2 + 18S)
bact_genus <- tax_glom(bact,"Genus")

tt <- as.data.frame(tax_table(bact_genus)) %>%
  dplyr::rename(Species = dplyr::last_col())

tax_table(bact_genus) <- tax_table(as.matrix(tt))
  

fung2 <- fung

sample_names(fung2) <- fung2@sam_data$sample_name %>% str_remove("F-")
full <- bind_phyloseq_taxa_simple(bact_genus,fung2)
...
fung_hubscores <- igraph::hits_scores()(fung_igraph2)
bact_hubscores <- igraph::hits_scores()(bact_igraph2)
...
all_plants_bact_stats <- readRDS("./Output/all_plants_bacteria_network_stats.RDS")
